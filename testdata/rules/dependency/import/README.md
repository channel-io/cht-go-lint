...
| `violations/` | A deliberately-broken repo (reverse-direction, cross-feature, undeclared-sibling, and `producer → internal/codec` where only `consumer` is granted the hoisted node). Every import marked `// WANT-VIOLATION` must be reported and nothing else; `TestViolationsFixture` asserts the exact set. |
